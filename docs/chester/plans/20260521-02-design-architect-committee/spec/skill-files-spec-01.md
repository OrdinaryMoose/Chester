...
- **`schema/`** — seven data-only files, word-limit exempt:
  - `schema/constraint-envelope.md` — five-field row shape, source-enum, provenance-enum, status-enum, prefix conventions. Sourced verbatim from `deliverables-locked-00.md` §"Three deliverables → Constraint Envelope".
...
### AC-7.3 — Skill registration discoverability honored

**Observable boundary:**
- `skills/setup-start/SKILL.md` is inspected for the "two-place sync" convention claimed by `skills/CLAUDE.md` and root `CLAUDE.md`. **Open finding for plan-build to resolve at planning time:** as of 2026-05-23, `skills/setup-start/SKILL.md` contains no explicit "Available Skills" section and does not name any individual skill (no mention of `design-committee`, `design-small-task`, `design-specify`, `plan-build`, etc.). The two-place-sync rule documented in CLAUDE.md appears to have drifted out of the current `setup-start/SKILL.md` body.
- Acceptable resolutions at plan-build:
  - **(a)** If `setup-start/SKILL.md` does carry a discoverable skill-listing structure not detected by name-grep, add a `design-architect-committee` entry there with description matching `skill.md` frontmatter.
  - **(b)** If the two-place-sync rule is genuinely dead, add the skill via whatever registration mechanism Chester actually uses (plugin manifest at `.claude-plugin/plugin.json` already auto-discovers via `skills/{name}/SKILL.md` filesystem layout, so registration may be by file presence only — no further action required).
  - **(c)** If the determination requires CLAUDE.md correction (the convention drifted), surface the drift to the user as a separate finding before committing.

**Given:** the build is in plan-build.
**When:** plan-build resolves the registration-mechanism question.
**Then:** plan-build records the determination in its `Decisions:` block; the choice is reflected in the implementing tasks.
...
- **SUPERSEDED (2026-05-23):** Same supersession as v00 — committee R2 abandoned the token-grammar design-specify line. Replaced by spec v03 at `spec/skill-files-spec-03.md`. Retained for audit trail.
