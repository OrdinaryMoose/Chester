...
- **`schema/`** — seven data-only files, word-limit exempt:
  - `schema/constraint-envelope.md` — six-field row shape (`concern_id`, `entry_id`, `source`, `body`, `provenance`, `status`), source-enum, provenance-enum, status-enum, prefix conventions. Sourced verbatim from `deliverables-locked-00.md` §"Three deliverables → Constraint Envelope".
  ...
  - `schema/coverage-map.md` — five-field row shape, status enum, status semantics. Sourced from `deliverables-locked-00.md` §"Three deliverables → Coverage Map".
  - `schema/phases-and-transitions.md` — five named phases, five named transitions, cascade timing rules, withdrawal exception. Sourced from `process-locked-00.md` §"Session phases", §"Transitions", §"Cascade handling — hybrid timing", §"Withdrawal handling".
  - `schema/procedures.md` — twelve named procedures, each with Mutates / Trigger / Gates / State one-liner fields. Sourced from `procedures-locked-00.md` (per-procedure subsections).
  - `schema/actors.md` — five-role inventory, twelve-procedure-to-actor mapping plus Dispatch Round, designer-surface-per-phase. Sourced from `actors-locked-00.md` (role inventory + mapping subsections).
  - `schema/integrity-rules.md` — five cross-artifact FK rules, three-condition session-close gate predicate. Sourced from `deliverables-locked-00.md` §"Cross-artifact integrity rules" and `process-locked-00.md` §"Session-close gate".
...
- `[CE-FIELD-SHAPE]` definition enumerates all six fields by name: `concern_id`, `entry_id`, `source`, `body`, `provenance`, `status`. (The five tokens above are concept-anchors, not a field count.)
...
- `[PHASE-TRANSITION-TABLE]` carries the load-bearing constraint that no transition fires automatically on a coverage condition; every advance is designer-triggered. Sourced from `process-locked-00.md:29`.
...
### AC-7.3 — Skill registration entry added to `setup-start/references/skill-index.md`

**Observable boundary:**
- `skills/setup-start/references/skill-index.md` contains an entry for `design-architect-committee` with a description matching the frontmatter of `skill.md`. The index file exists and already lists individual skills (`design-committee` at line 29, `design-small-task` at line 28, `design-specify` at line 30, `design-large-task` at line 27 — confirmed by ground-truth review 2026-05-23). The new entry is appended in the conventional position.
- The root `CLAUDE.md` and `skills/CLAUDE.md` claims about two-place sync point at `setup-start/SKILL.md` itself rather than the referenced index file. **Plan-build records a separate observation in its `Decisions:` block surfacing this CLAUDE.md drift for the designer to address in a future CLAUDE.md maintenance pass** — but the drift does not block this build's registration step.

**Given:** the build is complete.
**When:** a reader greps `setup-start/references/skill-index.md` for `design-architect-committee`.
**Then:** at least one match returned, located inside the skill-index body in the conventional position; description matches `skill.md` frontmatter.
...
- **01 (2026-05-23):** Adversarial spec review fixes. Corrected section-number references for `deliverables-locked-00.md` (file uses `### 1 / 2 / 3` and `## Cross-artifact integrity rules`, not §3.1/§3.2/§3.3/§3.4) and `process-locked-00.md` (file uses named top-level sections, not numbered subsections). Rewrote AC-7.3 to surface the documented two-place-sync convention as an open finding for plan-build — `skills/setup-start/SKILL.md` contains no "Available Skills" section as CLAUDE.md claims; plan-build must resolve the registration mechanism at planning time.
- **02 (2026-05-23):** Applied ground-truth review LOW findings as spec fixes (designer-directed override of skill rule that LOW = don't fix). (1) Components section now states `schema/constraint-envelope.md` has a six-field row shape (was "five-field"); AC-3.1 observable boundary now requires `[CE-FIELD-SHAPE]` to enumerate all six fields by name. (2) AC-3.4 observable boundary now requires `[PHASE-TRANSITION-TABLE]` to carry the "no automatic transitions; every advance is designer-triggered" load-bearing constraint from `process-locked-00.md:29`. (3) AC-7.3 rewritten — ground-truth review confirmed `setup-start/references/skill-index.md` exists and lists individual skills; the registration target is now named explicitly, with the CLAUDE.md drift surfaced as a plan-build `Decisions:` observation rather than an open-finding gate.
- **SUPERSEDED (2026-05-23):** Committee R2 review diagnosed token-grammar drift across all three versions. Replaced by spec v03 (Option B++) at `spec/skill-files-spec-03.md`. Retained for audit trail. Six-field CE correction + designer-triggered transitions clarification carry forward to v03 via Class-1 fact-correction errata per Decision 2a. CONVENE-MSG-PATTERN placement in schema/actors.md carries forward via Class-2 bounded-discretion per Decision 3a.
