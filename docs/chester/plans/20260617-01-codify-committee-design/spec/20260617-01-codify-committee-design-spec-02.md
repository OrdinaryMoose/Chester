...
**Confirmed clean — must remain unchanged (absence is a tested outcome):** all `CLAUDE.md` files; the repo-tracked project settings file `.claude/settings.chester.local.json` (pure config — `working_dir`/`plans_dir` only); `skills/spec-architect/SKILL.md` (committee path still skips it; remains accurate). The user-scoped `~/.claude/settings.chester.json` (budget config, verified pure config in round02) lives **outside the repo** and therefore cannot appear in a sprint diff — it is clean and out of scope, not a diff-asserted file.
...
**Observable boundary:**
- No `CLAUDE.md` file, the repo-tracked `.claude/settings.chester.local.json`, and `skills/spec-architect/SKILL.md` appear in the sprint diff. (The user-scoped `~/.claude/settings.chester.json` is outside the repo and cannot appear in a diff — it is not asserted here.)
