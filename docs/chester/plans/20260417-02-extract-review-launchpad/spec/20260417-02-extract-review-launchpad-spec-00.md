# Extract Review Launchpad — Specification
...
1. One-line file purpose
2. "Dispatch after:" trigger note
3. Fenced `Task tool` block containing:
   ...
   - `prompt:` — the full subagent prompt, containing:
     ...
     - What-to-check section — the five attack dimensions (structural integrity, execution risk, assumptions, contract and migration completeness, concurrency) from `plan-attack/SKILL.md` Step 2. Dimension headers and their primary bullet points are copied verbatim; intervening prose may be compressed to fit a subagent-prompt cadence.
     - Evidence standard — **verbatim** from `plan-attack/SKILL.md`: "Every finding must cite a specific file path, line number, or concrete code reference. If you cannot point to codebase evidence, drop the finding."
     - Output format — severity-grouped findings, 2–3 sentence risk assessment at the end. Verbatim from `plan-attack`.
4. "Reviewer returns:" footer listing: grouped findings, risk assessment, assumptions register
...
Same structure as `plan-attack-dispatch.md`. Content mapped from `plan-smell/SKILL.md`:

- Role statement ("You are a forward-looking code smell reviewer...")
- What-to-check — structural concerns (duplication, responsibility overload, unnecessary abstraction, deferred complexity) and coupling concerns (tight coupling, shotgun surgery, hierarchy problems, contract fragility). Plus the practical risks section (error paths, resource management, concurrency).
- Evidence standard — verbatim from `plan-smell`.
- Output format — verbatim from `plan-smell`.
...
Net changes: step 1 rewritten (read dispatch templates, not skill files), step 2 rewritten (substitute placeholder, not embed skill content), step 3 rewritten (launch per substituted template). Steps 4–8 identical to today's steps 3–7 plus the renumber.

### Files explicitly not changed

- `skills/plan-attack/SKILL.md` — remains a standalone, interactive skill. No `## Role` section, no trim. Its existing Step 1 ("The user will either point to a file, or have just finished planning…") already handles dispatch correctly from the subagent's perspective: when the subagent receives a pre-filled dispatch prompt, the plan path is already present and Step 1 has nothing to resolve.
- `skills/plan-smell/SKILL.md` — same rationale.
- `skills/plan-build/plan-reviewer.md` — already the canonical pattern; no change.
- `skills/setup-start/SKILL.md` — `plan-attack` and `plan-smell` remain listed with unchanged descriptions; no entry needs updating.
...
5. **Verbatim preservation of evidence standard and output format.** These are quality-determining — the refactor condenses the Step 2 instructions but keeps evidence rules and output schema exact. Drift here causes subagent output degradation.
6. **Dual-source content sync is a manual discipline.** If the evidence standard or output format in `plan-attack/SKILL.md` changes, the dispatch template must be updated in the same commit. A one-line note at the very top of each dispatch template (above the one-line purpose) reminds readers: *"Evidence standard and output format must match `plan-attack/SKILL.md` — update both together."*
...
- **Lens-content forking vs. embedding** — the chosen approach forks into a purpose-built dispatch prompt but does not fork the lens skill itself. This is intentional; the lens remains standalone-invocable.
...
1. `skills/plan-build/plan-attack-dispatch.md` exists and contains a valid subagent prompt with `[PLAN_FILE_PATH]` placeholder, evidence standard, and output format.
2. `skills/plan-build/plan-smell-dispatch.md` exists and satisfies the same shape.
