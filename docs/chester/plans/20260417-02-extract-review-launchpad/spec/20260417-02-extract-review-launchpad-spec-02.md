# Extract Review Launchpad — Specification (v02)

> **Revision notes:**
> - **v00 → v01:** Ground-truth review of v00 flagged that inline paraphrases of `plan-attack`'s evidence standard and output format diverged from the actual lens content (paraphrases dropped content; "severity-grouped findings" contradicted the lens's explicit "whatever severity scale feels natural" guidance). v01 replaced inline content quotes with direct file:line references.
> - **v01 → v02:** Ground-truth review of v01 flagged MEDIUM asymmetric line-range convention across the two lenses (attack ranges included `##` section headers; smell ranges excluded them; one range extended one line into the next section). v02 normalizes all content ranges to a **body-only convention** — exclude `###`/`##` section headers, stop before the next section — so the implementer produces symmetric dispatch templates without header leakage. No architectural change.
...
**Line-range convention — body-only.** All line ranges below point at **body content** only: they start at the first line of content (not the `##`/`###` section header) and end at the last line of that section's body (not a trailing blank, not the next section's header). The dispatch template provides its own `## Evidence Standard` and `## Output` section headers (matching `plan-reviewer.md`'s fenced-prompt layout); the body content is what gets pasted underneath. This convention is applied symmetrically across both lenses.
...
4. Fenced `Task tool` block containing:
   ...
   - `prompt:` — the full subagent prompt, containing:
     ...
     - **What to check** — copied verbatim from `plan-attack/SKILL.md:38-76` (the five attack dimensions: structural integrity, execution risk, assumptions and edge cases, contract and migration completeness, concurrency and thread safety, including the "For each dimension, verify claims against actual code" closing instruction). Excludes the `### Step 2 — Attack the plan` header at line 36.
     - **Evidence standard** — copied verbatim from `plan-attack/SKILL.md:80-88`. This includes the bullet structure, the "single most important rule" clause, and the TRUE/FALSE/UNVERIFIABLE assumptions guidance. Do not collapse into a single-sentence paraphrase. Excludes the `## Evidence Standard` header at line 78.
     - **Output guidance** — copied verbatim from `plan-attack/SKILL.md:92-101`. Note: the lens explicitly rejects prescribed taxonomies ("Use whatever severity scale and format feels natural — the goal is clarity, not taxonomy compliance. Group related findings when they share a root cause."). The dispatch template must preserve this open-ended guidance, not pre-specify a severity table or schema. The lens also includes the "Note any assumptions the plan makes that you verified as correct" clause and the "End with a brief risk assessment … 2-3 sentences" clause — both must be present. Excludes the `## Output` header at line 90.
...
- **What to check** — copied verbatim from `plan-smell/SKILL.md:58-87` (structural concerns: duplication, responsibility overload, unnecessary abstraction, deferred complexity; coupling and change-propagation concerns: tight coupling, shotgun surgery, hierarchy problems, contract fragility; and the practical risks sub-section: error paths, resource management, concurrency). Excludes the `### Step 2 — Review for smells` header at line 56 and stops before the `## Evidence Standard` header at line 89.
- **Evidence standard** — copied verbatim from `plan-smell/SKILL.md:91-97`. This includes the three-bullet structure. The third bullet — *"Whether the plan explicitly acknowledges the risk (if so, note it but don't penalize)"* — is load-bearing for preventing finding-inflation and must be preserved. Excludes the `## Evidence Standard` header at line 89 and stops before the `## Output` header at line 99.
- **Output guidance** — copied verbatim from `plan-smell/SKILL.md:101-107`. Same "whatever feels natural" open-endedness as plan-attack. Preserve the "group related findings" and "2-3 sentence risk assessment" clauses. Excludes the `## Output` header at line 99 and stops before `### Step 3 — Stop` at line 109.
...
4. **Verbatim-content check.** For each of the six content regions copied from a lens (attack what-to-check, attack evidence standard, attack output guidance, smell what-to-check, smell evidence standard, smell output guidance), diff the dispatch template region against the referenced line range in the lens `SKILL.md`. Ranges are body-only (no `##`/`###` section headers); the dispatch template provides its own section headers matching `plan-reviewer.md`'s layout. The body text must match modulo markdown heading level and the subagent-prompt indentation of the fenced block. Any divergence is a bug.
...
7. **Verbatim-content check passes:** for each of the six copied content regions (attack what-to-check, attack evidence standard, attack output guidance, smell what-to-check, smell evidence standard, smell output guidance), the dispatch template text matches the body-only lens line range modulo markdown heading level and fenced-block indentation. No `##`/`###` section headers from the lens appear in the copied body; no paraphrasing, no bullet dropping, no taxonomy imposition.
