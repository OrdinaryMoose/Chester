...
> **Revision note (spec-01).** Supersedes spec-00 after the inline adversarial review. Two fixes: (1) the no-collapse / "surface options" doctrine is reconciled across **all five** occurrences in `team-lead.md`, not just Behavioral Constraints — Presentation Rules added to the edit set (HIGH); (2) the locked decision-communication packet format is correctly located in `team-lead.md` §Visible Surface (the four-block Information Packet Format + exemplar), not in the scribe's `artifact-template.md` — AC-2.2 and C-LOCKED corrected (MEDIUM).
>
> **Revision note (spec-02).** Supersedes spec-01 after the ground-truth review. One fix: the "two-surface" term-collision behind Constraint C-NAMING was misattributed to sprint `20260606-01` (which never uses the term). The literal "two-surface" usage lives in sprint `20260521-02-design-architect-committee`; attribution corrected throughout, and the inaccurate "artifact-boundary" characterization (tied to the wrong sprint) is dropped (MEDIUM).

## Goal

Realign the `design-committee` skill so each round's terminal object is the most-informative answer to the designer's question — which may converge, preserve a legitimate split, or be partial — with its gaps named, rather than a decision menu the designer must resolve. The decision-seeking interaction becomes an instrument for extracting designer-held value-judgments the committee cannot derive itself; the answer is the deliverable. A team-lead authority guard keeps a no-opinion team-lead honest while it authors that answer: every answer-body assertion carries a warrant (evidence, logic, or in-scope designer premise) or is demoted to a gap, and the warrants are recorded on disk so the designer can audit them. The realignment lands entirely in team-lead-owned files and the orchestration skill, leaving the member agents, the Scribe contract, the locked decision-communication packet format, and all rigid internal contracts untouched.
...
- **`skills/design-committee/references/team-lead.md`** (PRIMARY; version v0009 → v0010). Owns the five principles, the revised per-round loop, the output-surface split, and the complete authority guard. Sections changed:
  - *Conversation Loop / Behavioral Constraints* (line ~121) — replace "Do NOT collapse irreducible splits when split is the finding" with the P1 answer-shape rule (converge / preserved-split / partial, chosen to lose the least information; collapse permitted only when warranted; collapse not required). Add: count is not a warrant; strict premise scope; the above-threshold gap trichotomy; designer-determined sufficiency (P3) as the sole termination trigger.
  - *Reconcile the no-collapse / decision-menu doctrine across ALL its occurrences in `team-lead.md`* so nothing contradicts P1. Confirmed sites: line ~121 (Behavioral Constraints), ~191 (Decision Package / Split adjudication — "Do NOT collapse to single recommendation when split is the finding"), ~304 (Consolidation Rules — "Irreducible split → … do NOT collapse to single recommendation"), ~308 (Presentation Rules — "**Surface options, not verdict.**"), ~320 (Self-Evaluation — "Did I collapse irreducible member disagreement?"). The reframing: a warranted/irreducible split is still never force-collapsed, but the round's output is an *answer* (which may legitimately be a preserved split), not a menu of options. The line ~308 phrase "Surface options, not verdict" must be rewritten — it is the decision-menu framing P1 overturns. (The doctrine is confined to `team-lead.md`; `SKILL.md` and `committee-analysis-round-format.md` do not restate it — verified.)
  - *Visible Surface* (lines ~148–274) — add the output-surface split statement (see Constraint C-NAMING): the decision-communication packet format is locked and unchanged and is used only when seeking a decision; the end-of-turn session artifact has no mandated format. Add the P2 rule to the existing Decision Package / Split adjudication subsection (line ~191): a split yields the pointed question each side raises against the other, surfaced one at a time, pre-answered where possible. **The four-block Information Packet Format (Summary / Information Package / Decision Package / Team-Lead Comments) and the Style Exemplar are NOT structurally modified — additions are made around them (C-LOCKED).**
  - *Internal Discipline / Consolidation Rules* (line ~304) — add the warrant test (name a warrant for every answer-body assertion; unwarranted → demote to gap), the C2 firewall (opinion only in the fenced Recommendation block), and the C1 audit (every collapse displays its warrant). Add the warrant-on-disk rule: warrants are recorded in the team-lead's own `verdict.md` / `alignment-map.md` so they are auditable, not held only in context.
  - *Internal Discipline / Self-Evaluation* (line ~320) — add an "Authority Guard" self-eval sub-block: warrant-coverage check, count-not-a-warrant check, strict-premise-scope check. Reconcile the existing "Did I collapse irreducible member disagreement?" check with the new answer-shape framing.
...
- **`skills/design-committee/SKILL.md`** (MINOR; version v0019 → v0020). Update Phase 2 Capture Question (line ~66) and Phase 4 Modes (lines ~129–132) so the loop is framed as interview-to-resolution terminating on designer sufficiency (P3/P5), not a fixed one-round/two-round count; two-round Delphi remains an available technique, not the ceiling. Carry the output-surface split forward as orchestration framing where SKILL.md references the decision packet.
- **`skills/design-committee/references/committee-analysis-round-format.md`** (LIGHT; version bump — read current version at plan time, increment by one). Add one paragraph documenting that `verdict.md` and `alignment-map.md` may carry a warrant block and an `answer-shape` marker, and resolve the term collision per Constraint C-NAMING (the realignment's output-surface split is a distinct concept from this sprint-lineage's existing "two-surface" usage in sprint 20260521-02-design-architect-committee).

**Untouched by construction (deferrals + rigid contracts):** the four advocacy agent files, `design-committee-researcher.md`, `design-committee-consolidator.md`, `design-committee-scribe.md`, `references/member-protocol.md`, `references/artifact-template.md` (the **scribe's** artifact template — Summary/Verdict/Rationale/Dissent Record/Deferred-Open; this is NOT the decision-communication packet format and is a deferred file), `references/skill-contract.md`, and `skills/util-design-partner-role/SKILL.md`.
...
4. **Output-surface split at Present:** when the team-lead seeks a decision, it communicates via the locked decision-communication packet — the four-block Information Packet Format defined in `team-lead.md` §Visible Surface (unchanged structure). The end-of-turn session artifact itself has no mandated format — it is whatever information fits the question.
...
- **Residual decision-menu doctrine after edit.** Failure mode: an un-reconciled occurrence of "do NOT collapse" or "surface options, not verdict" survives in `team-lead.md` and contradicts P1. Response: the edit reconciles all five confirmed sites; AC-1.3 makes the absence of contradictory phrasing observable.
...
- **Term collision ("two-surface").** Failure mode: a reader conflates the realignment's output-surface split with the existing "two-surface" usage in sprint 20260521-02-design-architect-committee. Response: Constraint C-NAMING mandates distinct terms in all touched files.
...
- **Doctrine-reconciliation grep.** `grep -ni "surface options\|do not collapse\|collapse irreducible" team-lead.md` after the edit returns only phrasings consistent with P1 (no surviving decision-menu framing).
...
- **Decision-format-preservation check.** The four-block Information Packet Format and the Style Exemplar in `team-lead.md` §Visible Surface are structurally identical before and after (additions only, no block removed or reshaped).
...
- **C-LOCKED — Decision-communication packet format unchanged.** The locked format is the four-block Information Packet Format (Summary / Information Package / Decision Package / Team-Lead Comments) and its Style Exemplar, defined in `team-lead.md` §Visible Surface (lines ~148–274). Its block structure and exemplar are not modified; the realignment only adds the output-surface split statement and the P2 split-question rule around them. The scribe's `artifact-template.md` is a separate, deferred file and is also byte-unchanged.
...
- **C-NAMING — Distinct term for the output-surface split.** The realignment's decision-communication-vs-session-artifact concept (P4) MUST use a term distinct from the existing "two-surface" usage in sprint 20260521-02-design-architect-committee. The touched files state the chosen term once and use it consistently.
...
### AC-1.3 — No-collapse / decision-menu doctrine reconciled across all sites

**Observable boundary:**
- `grep -ni "surface options\|do not collapse\|collapse irreducible\|not verdict" skills/design-committee/references/team-lead.md` → returns no phrasing that frames the round output as a menu of options rather than an answer; every surviving mention of not-collapsing a split is consistent with P1.
- Reading `team-lead.md` Presentation Rules → the phrase "Surface options, not verdict" is gone or rewritten to the answer-delivery framing.

**Given:** the realignment is applied.
**When:** all five confirmed doctrine sites (Behavioral Constraints ~121, Split adjudication ~191, Consolidation Rules ~304, Presentation Rules ~308, Self-Evaluation ~320) are inspected.
**Then:** none contradicts P1; the preserve-a-warranted-split intent is retained, the decision-menu framing is removed.
...
- Reading the touched files → the P4 concept uses a term distinct from the existing "two-surface" usage in sprint 20260521-02-design-architect-committee (C-NAMING), used consistently.
...
### AC-2.2 — Locked decision-communication format preserved (correct location)

**Observable boundary:**
- Reading `team-lead.md` §Visible Surface → the four-block Information Packet Format (Summary / Information Package / Decision Package / Team-Lead Comments) and the Style Exemplar are structurally identical to the pre-edit version; only additive statements (output-surface split, P2 split-question rule) surround them.
- `git diff` → the scribe's `references/artifact-template.md` is byte-unchanged (separate deferral).

**Given:** the realignment is applied.
**When:** the team-lead seeks a designer decision.
**Then:** the communication uses the existing four-block format with no structural change to it or its exemplar; the misdirected "artifact-template.md is the format" assumption from spec-00 is corrected — that file is the scribe's artifact, not the decision-communication format.
...
**Observable boundary:**
- Reading `team-lead.md` → gaps are surfaced one at a time; for a split, the team-lead poses the pointed question each side raises against the other, pre-answered where possible. The rule is hosted at the existing Decision Package / Split adjudication subsection.
...
**Observable boundary:**
- Frontmatter → `team-lead.md` v0009→v0010; `SKILL.md` v0019→v0020; `committee-analysis-round-format.md` bumped by one from its current version.
...
<!-- created-at: 2026-06-09T13:34:38Z -->
