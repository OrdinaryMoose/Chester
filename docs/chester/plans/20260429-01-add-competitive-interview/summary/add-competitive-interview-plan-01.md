...
**Spec:** `docs/chester/working/20260429-01-add-competitive-interview/spec/add-competitive-interview-spec-03.md`
**Revision history:** v00 (initial draft); v01 (plan-hardening Mitigations A-E applied — full SKILL.md template-site patching, capture_thought-based Resume Protocol, mid-chain failure + polite-collapse procedural handlers, orchestrator-printing prose + AC-5.2 test, fabricated stance principle correction, tightened grep anchors)
...
## Task 3: Specify Round-Zero, Round Sequence, Per-Round Phases, and Orchestrator-Side Dispatch Printing

**Type:** code-producing
**Implements:** AC-1.3, AC-1.4, AC-1.5, AC-5.2
**Decision budget:** 3 (exact wording for sequential chain semantics; whether to name R5 "synthesis" or "ratification round"; how to phrase the orchestrator-printing convention reference)
**Must remain green:** AC-1.1, AC-1.2 (regression: file still exists with all sections)

**Files:**
- Modify: `skills/design-large-task/references/team-interview-flow.md` (Round-Zero, Round Sequence, Per-Round Phases sections — including orchestrator-side dispatch/completion printing prose folded into pole-dispatch instructions)
- Test: `tests/test-ac-1-3-round-opener-rotation.sh`, `tests/test-ac-1-4-sequential-chain.sh`, `tests/test-ac-1-5-r5-parallel-synthesis.sh`, `tests/test-ac-5-2-orchestrator-printing.sh`

**Steps (TDD):**

- [ ] **Step 1: Write the failing tests**

`tests/test-ac-1-3-round-opener-rotation.sh` — anchored to a Round Sequence section to avoid false-positive matches on isolated letters (`\bN\b` matched any word boundary, e.g. "N/A"):
...
FLOW="skills/design-large-task/references/team-interview-flow.md"
ERRORS=0
# Extract the Round Sequence section only (avoid scanning unrelated prose)
SECTION=$(awk '/^## Round Sequence/,/^## /' "$FLOW" | sed '$d')
...
`tests/test-ac-1-4-sequential-chain.sh` — anchored to Per-Round Phases section, requires explicit "sequential chain" two-word phrase:
...
FLOW="skills/design-large-task/references/team-interview-flow.md"
ERRORS=0
SECTION=$(awk '/^## Per-Round Phases/,/^## /' "$FLOW" | sed '$d')
...
`tests/test-ac-1-5-r5-parallel-synthesis.sh` — anchored to a Round 5 / Synthesis section to avoid false-positive matches on the word "parallel" elsewhere:
...
`tests/test-ac-5-2-orchestrator-printing.sh` — verifies the orchestrator-side dispatch/completion printing convention is referenced in pole-dispatch instructions:
...
FLOW="skills/design-large-task/references/team-interview-flow.md"
ERRORS=0
# Reference to orchestrator-side / dispatch / completion printing convention
...
# Tied to per-round pole-dispatch instructions (Per-Round Phases section)
SECTION=$(awk '/^## Per-Round Phases/,/^## /' "$FLOW" | sed '$d')
...
# Reference to source sprint (2026-03-31) for traceability
...
bash tests/test-ac-5-2-orchestrator-printing.sh   # FAIL
```
...
- **Orchestrator-side dispatch/completion printing (per sprint 2026-03-31 — Subagent Progress Surface):** in the Per-Round Phases section, add explicit prose stating that **every pole subagent dispatch must surface dispatch and completion lines on the main screen via orchestrator-side printing** — not buried in subagent output panels. Format: lead prints `Dispatched: design-large-task-step-b-{pole}:R{N}-{phase}` when dispatching, and `Completed: design-large-task-step-b-{pole}:R{N}-{phase}` when the pole returns. This applies to all per-phase pole dispatches in R1–R4 and the parallel synthesis attacks in R5. Cite the source sprint (`2026-03-31` — Subagent Progress Surface) so the convention's traceability is preserved.
...
bash tests/test-ac-5-2-orchestrator-printing.sh   # AC-5.2: PASS
...
git add skills/design-large-task/references/team-interview-flow.md \
        ...
        tests/test-ac-5-2-orchestrator-printing.sh
git commit -m "feat(design-large-task): specify team-interview round mechanics with orchestrator-side dispatch printing"
```
...
`tests/test-ac-5-1-voice-discipline-markers.sh` — anchored greps for the named markers, requires the canonical phrasing alongside the C1/C2 labels to avoid false-positive matches on isolated `C1`/`C2` tokens (markdown columns, headings):
...
FLOW="skills/design-large-task/references/team-interview-flow.md"
ERRORS=0
# C1 with its canonical phrase "Externalized Coverage"
...
# C2 with its canonical phrase "Fact Default with Marked Departures" OR shorthand "Fact/Assumption/Opinion"
...
## Task 5: Specify Transcript Schema, Validity-Test Checklist, Termination Rules, Error Handling, Resume Protocol

**Type:** code-producing
**Implements:** AC-1.7, AC-1.8, AC-1.9 (plus spec Error Handling lines 72–73 — mid-chain failure recovery + polite-collapse re-prompt — covered by prose without a dedicated AC test, per Mitigation C)
**Decision budget:** 4 (transcript layout granularity; validity-test specifics per category; per-round `capture_thought` tagging convention for Resume; mid-chain retry / polite-collapse procedural detail)
**Must remain green:** AC-1.1, AC-1.2 (regression)

**Files:**
- Modify: `skills/design-large-task/references/team-interview-flow.md` (Transcript Schema + Validity-Test Checklist + Termination Rules + new Error Handling subsection + Resume Protocol sections; add per-round `capture_thought` calls into Per-Round Phases recommendation step)
...
- **Error Handling subsection (per spec lines 72–73, Mitigation C):** add a peer subsection covering two procedural handlers. (a) **Mid-chain failure recovery:** if a pole subagent returns an error mid-Phase-2 chain, lead retries the failed pole once with the same chain context; if retry fails, lead notes the failure in the transcript, skips the failed pole's opposition, and continues to the next chain step. The recommendation phase records `"chain partial"` as a quality flag. (b) **Polite-collapse re-prompt:** lead detects when no real attacks landed in a round (no opposer produced disagreement; counter-arguments are all "agreed"); re-prompts the weakest opposer with directive to defend its lens position from first principles, not respond to opener. One re-prompt per round; if still polite, recommendation marks the round as `"shallow"` (flagged in process evidence).
- **Per-round `capture_thought` calls (per Mitigation B — supports Resume Protocol):** add a step at the close of each round's recommendation phase: lead calls `capture_thought()` with tag `team-interview-r{N}-recommendation`, stage `Understand`, content summarizing the round (opener, surviving statement, status). This writes incrementally during the Understand Stage (existing capture_thought behavior — no read-only-discipline violation) and gives Resume Protocol a recoverable trail. R5 ratification close also captures `team-interview-r5-ratification` plus the existing `understanding-confirmed` capture.
- **Resume Protocol (Mitigation B — pivot to capture_thought history):** when `ACTIVE_UNDERSTANDING_MCP=team-interview` and `understanding-confirmed` thought is absent, recovery path calls `get_thinking_summary()` and reads the `team-interview-r{N}-*` tags written so far. From the most recent round capture, lead reconstructs which round was active, which poles had spoken, and which statements were surviving. Lead summarizes current debate state in domain language and resumes the next round per Per-Round Phases. (No process-evidence transcript file is read during resume — the transcript is written at Phase 5 Closure, not incrementally during the Understand Stage.)
...
3. **Software Architect persona section** — inherit from `util-design-partner-role` Stance Principles (lines 130–137 of that skill, verbatim): "Be opinionated. Read code as design history. Think in trade-offs. Evaluate boundaries as choices. Align architecture to intent." (Mitigation E correction: original draft listed a fabricated sixth principle "Operate across abstraction levels" that does not appear in `util-design-partner-role`. Use only the five real principles.) Apply these traits while playing your lens.
...
## Task 9: Update `design-large-task/SKILL.md` template-path patches, Phase 3 / Phase 4 conditionals, HARD-GATE conditional, Solve Stage opening note, capture_thought-based Resume Protocol branch

**Type:** code-producing
**Implements:** AC-3.2, AC-3.3
**Decision budget:** 5 (template-path generalization phrasing across 4 sites; HARD-GATE conditional language; Phase 3 step 2 conditional; Solve Stage opening reconciliation phrasing; Resume Protocol capture_thought-based branch)
**Must remain green:** AC-3.1 (regression: swap-line still has team-interview); AC-6.2 (regression: Phases 1, 2, 5 untouched)

**Files:**
- Modify: `skills/design-large-task/SKILL.md` — Mitigation A patches at lines 71 (intro prose template), 216–231 (HARD-GATE block), 269 (Phase 4 Two-Stage Model template), 326 (Understand Stage Per-Turn Flow template); plus Phase 3 step 2 (~line 244); Phase 4 Understand Stage opening note; Phase 4 Solve Stage Opening step 1 (~line 399); Resume Protocol section (lines 732–739)
...
`tests/test-ac-3-2-skill-phase-conditional.sh` — extended (Mitigation A) to verify all four template sites are patched and the HARD-GATE block has a team-interview conditional:
...
# Conditional on team-interview noted somewhere
...
# Template sites patched: count occurrences of the un-generalized template path.
# Pre-Mitigation-A there were ~6 occurrences (lines 71, 89, 226, 244, 269, 326).
# Post-patch the literal `{ACTIVE_UNDERSTANDING_MCP}-mcp-flow.md` should remain only where it correctly resolves
# (i.e. in code-style examples) AND each site that mentions Understanding flow loading must also acknowledge the
# team-interview path (no `-mcp-` infix). Test: every paragraph that references the template path also references
# team-interview-flow.md within reasonable proximity.
# Pragmatic check: the count of references to team-interview-flow.md is at least 4 (one per patched site + Phase Map step 3).
...
# HARD-GATE conditional (Mitigation A): the HARD-GATE block must include language acknowledging that for team-interview,
# the next action is loading team-interview-flow.md and constructing the Round-Zero context packet (NOT initialize_understanding).
...
Modify `skills/design-large-task/SKILL.md` — eight distinct sites (six existing in v00 plus four added by Mitigation A):

a. **Line 71 prose template (Mitigation A)** — currently "the active MCP's per-turn cycle and transition criteria are specified in `references/{ACTIVE_UNDERSTANDING_MCP}-mcp-flow.md`". Generalize to: "the active flow's per-turn cycle and transition criteria are specified in `references/{ACTIVE_UNDERSTANDING_MCP}-mcp-flow.md` (or `references/team-interview-flow.md` when `ACTIVE_UNDERSTANDING_MCP=team-interview` — first non-MCP flow file uses no `-mcp-` infix)." Apply this generalization at each of the four template sites (lines 71, 226, 269, 326).

b. **HARD-GATE block lines 216–231 (Mitigation A — critical)** — the HARD-GATE currently mandates "Your next action is `initialize_understanding`. Nothing else." and points at the `*-mcp-flow.md` template. Add a conditional inside the HARD-GATE block immediately after the existing "Your next action is `initialize_understanding`. Nothing else." sentence: "**Conditional for `team-interview`:** when `ACTIVE_UNDERSTANDING_MCP=team-interview`, your next action is loading `references/team-interview-flow.md` and constructing the Round-Zero context packet from Phase 1–3 exploration outputs — there is no `initialize_understanding` call to make (team-interview has no MCP server). The HARD-GATE still applies: no designer-facing turn is permitted until that loading and packet construction is complete." This resolves the contradiction plan-attack flagged.

c. **Phase Map step 3 (line 89)** — already references `references/{ACTIVE_UNDERSTANDING_MCP}-mcp-flow.md`. Add the same generalization as (a) so the Phase Map self-documents.

d. **Phase Map step 4 (line 90)** — currently "Initialize understanding MCP — execute the Round-Zero / Round-One initialization steps described in the active flow reference. **No designer-facing turn is permitted until initialization completes.**" Add a conditional: "When `team-interview` is the active flow, no MCP `initialize_understanding` call runs (no MCP server). Round-Zero context-packet construction still executes per the flow reference's Round-Zero Initialization section."

e. **Phase 3 step 2 (line 244)** — currently "Execute initialization per the active flow reference. … Use the state file path: `{CHESTER_WORKING_DIR}/{sprint-subdir}/design/{sprint-name}-understanding-state.json`." Add a parenthetical: "(For `team-interview`, no state file is created — there is no Understanding MCP. The flow file's Round-Zero Initialization section specifies what to do instead: build the context packet from Phase 1–3 outputs.)"

f. **Phase 4 Two-Stage Model template (line 269) and Understand Stage Per-Turn Flow template (line 326) (Mitigation A)** — apply the same generalization as (a) so all four template-resolution sites remain coherent for team-interview.

g. **Phase 4 Understand Stage** — add a one-paragraph note at the top of the section explaining that when `team-interview` is the active flow, the per-turn cycle is the four-pole sequential-chain debate per `references/team-interview-flow.md`, not single-agent saturation scoring. The Stage Transition logic (`understanding-confirmed` thought capture) is the same — team-interview-flow.md fires that capture at the end of Round 5 ratification.

h. **Phase 4 Solve Stage Opening step 1 (line 399)** — currently: "Problem statement: polish, readback, confirm." Append a conditional note: "**When `team-interview` was the active flow,** the problem statement was already polished and ratified during Round 5 synthesis. Replace polish/readback/confirm with a single confirmation prompt: 'The team ratified this statement — confirm or revise?' If the designer revises, re-enter Round 5 synthesis with the revision; otherwise proceed to step 2."

i. **Resume Protocol (line 732) — Mitigation B (pivoted from process-evidence to capture_thought)** — add a team-interview branch. Current step 2: "If absent: the Understand Stage was active. Call `get_understanding_state` …". Modify to: "If absent AND `ACTIVE_UNDERSTANDING_MCP` is `classic` / `problemfocused` / `architectural`, the Understand Stage was active under an MCP — call `get_understanding_state` with the state file path. **If absent AND `ACTIVE_UNDERSTANDING_MCP=team-interview`, the team debate was in progress — call `get_thinking_summary()` and read the `team-interview-r{N}-recommendation` thoughts captured incrementally during the rounds (per the flow file's per-round `capture_thought` step). Reconstruct from the most recent recommendation: which round was active, which poles had spoken, which statements were alive/wounded/dead. Summarize current debate state in domain language and resume the next round per `references/team-interview-flow.md`. (No process-evidence transcript file is read — that file is written only at Phase 5 Closure, not incrementally during the Understand Stage.)"
