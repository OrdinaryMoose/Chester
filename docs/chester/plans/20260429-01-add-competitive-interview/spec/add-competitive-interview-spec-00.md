...
**Modified files:**
- `skills/design-large-task/SKILL.md` — swap-line block adds `team-interview` option; Phase 3 Round One MCP-init step becomes conditional on flow type; Phase 4 Understand Stage gains note that team-interview-flow.md drives the stage when active; Resume Protocol gains team-interview branch.
...
**New test files (one per AC group):**
- `tests/test-team-interview-flow-file.sh` — covers AC-1.X (flow file structure)
- `tests/test-team-interview-pole-agents.sh` — covers AC-2.X (pole agent definitions)
- `tests/test-team-interview-skill-integration.sh` — covers AC-3.X (SKILL.md changes)
- `tests/test-team-interview-fork-policy.sh` — covers AC-4.X (fork policy)
- `tests/test-team-interview-voice-discipline.sh` — covers AC-5.X (C1/C2 markers, persona)
- `tests/test-team-interview-composition.sh` — covers AC-6.X (composition preservation)

## Data Flow

1. **Entry from Phase 3.** Round One presentation completes. Lead reads `ACTIVE_UNDERSTANDING_FLOW` from SKILL.md swap-line. If `team-interview`, lead loads `references/team-interview-flow.md` instead of an `*-mcp-flow.md` reference. No Understanding MCP initialization.
2. **Round-Zero context packet construction.** Lead distills Phase 1–3 outputs (problem domain, prior-art findings, codebase exploration, framing + gap map) into a focused context packet. Packet is the prompt prefix every pole subagent receives in addition to its phase-specific instructions.
...
7. **Transition to Phase 5 (Solve Stage).** Solve Stage opens by reading the handoff artifact in place of Understanding MCP saturation history. Necessary-conditions proof seeds from consensus evidence; exit criteria become collapse tests.

## Error Handling

- **Mid-chain failure** (pole subagent returns error mid-Phase-2 chain): Lead retries the failed pole once with the same chain context. If retry fails, lead notes the failure in the transcript, skips the failed pole's opposition, and continues to the next chain step. The recommendation phase records "chain partial" as a quality flag. Mitigation rationale: completing the round with a documented gap is more useful than discarding the round entirely.
- **Polite collapse** (round ends with no real attacks landing — all opposers concede or echo opener): Lead detects (no opposer produced disagreement; counter-arguments are all "agreed"); re-prompts the weakest opposer with directive to defend its lens position from first principles, not respond to opener. One re-prompt per round. If still polite, recommendation marks the round as "shallow" — flagged in process evidence.
- **Stalled deadlock** (Round 5 ratification fails after one revision pass): Designer arbitrates. Designer either rules in favor of the blocking pole (reopen with new round, possibly killing the consolidated draft) or forces ratification with logged dissent (statement passes with weakness flagged in handoff).
- **Stage failure** (all four R1–R4 statements end "dead"): Lead reports "no problem statement survived debate" in the designer-facing turn. Designer either reframes original sprint scope and restarts the stage or accepts the finding ("problem isn't ready for design").
- **Composition fragility** (Solve Stage discovers it depends on Understanding MCP saturation history rather than just the handoff artifact): Out of scope to fix here. Spec mandates a verification read of Solve Stage's evidence consumption (Phase 5 opening steps) before merging — if Solve has hard dependencies on MCP fields, a shim is added in a follow-on sprint.

## Testing Strategy

Tests are bash scripts (project convention; see `tests/test-*.sh`). Each AC has a corresponding test stub keyed by skeleton ID `ac-{N-M}-{slug}`. Tests are structural: file existence, frontmatter shape, required-section presence in markdown files, swap-line content, fork-policy table content. No runtime exercise of the flow itself — that's validated only by running a real sprint with `ACTIVE_UNDERSTANDING_FLOW: team-interview` set, which is a manual acceptance gate not a unit-testable property.
...
- Validation that the classic flow still works: a regression test confirming `references/classic-mcp-flow.md` is unmodified and the swap-line still accepts `classic`.
...
- **Three-section handoff format must align with Optimize Throughput proposal** _(normative — sprint 20260417-03 approved)_: problem statement / consensus evidence / exit criteria, with evidence organized by type (codebase / practitioner-friction / philosophy / industry) and attributed to source pole
- **C1 (externalized coverage) and C2 (fact/assumption/opinion) markers must apply to pole transcripts and ratifications** _(normative — sprint 20260425-01 approved, Convergence Model voice discipline)_
- **Pole subagent system prompts must inherit Software Architect persona traits** _(normative — sprint 2026-03-27 shipped, Socratic Interview persona)_
- **Per-pole dispatch must surface dispatch/completion lines on main screen via orchestrator-side printing** _(normative — sprint 2026-03-31 shipped, Subagent Progress Surface)_
- **References file naming follows existing convention** _(structural — `{name}-mcp-flow.md` for MCP flows; `{name}-flow.md` for non-MCP flows; brief D6)_
- **Sprint name file prefix uses three-word portion only** _(structural — `util-artifact-schema`)_
- **Phases 1–3 and Phase 5 of `design-large-task/SKILL.md` body remain unchanged** _(structural — brief D5)_

## Non-Goals

- **Framing Convergence MCP** — not building. D3 explicit: lightweight first; instrument only after pattern proves itself.
- **Modifying Solve Stage internals** — not touching `Phase 5` or `references/proof-mcp-flow.md` (or whatever Solve uses). Solve consumes the new handoff artifact unchanged.
- **Migrating existing sprints to `team-interview`** — `classic` remains the default; existing flows unaffected.
- **`plan-build` / `execute-write` team-mode** — out of scope; pattern can be revisited for review-phase skills if Step B succeeds.
...
- **Renaming the existing swap-line variable from `ACTIVE_UNDERSTANDING_MCP` to `ACTIVE_UNDERSTANDING_FLOW`** — brief uses `ACTIVE_UNDERSTANDING_FLOW`; if existing SKILL.md uses `ACTIVE_UNDERSTANDING_MCP`, adopt whatever is currently there; the swap-line semantics matter, the variable name does not. Ground-truth review will confirm the actual variable name in SKILL.md and the spec accepts either.
...
**Observable boundary:**
- The Round Sequence section enumerates R1 with N (Innovator) as opener
- R2 with S (Conservator) as opener
- R3 with E (Purist) as opener
- R4 with W (Pragmatist) as opener
- R5 as the synthesis round (no single opener)

**Given:** the flow file exists
**When:** the test inspects the Round Sequence section
**Then:** the four R1–R4 openers map exactly to N, S, E, W in that order, and R5 is identified as a synthesis round
...
**Observable boundary:**
- The Per-Round Phases section's Phase 2 (opposing arguments) includes language indicating that opposer-2 receives opposer-1's statement and opposer-3 receives both opposer-1 and opposer-2's statements (i.e., the chain is sequential, not parallel)
...
**Then:** parallel attack semantics are explicit (R5 differs from R1–R4 sequential chain)
...
**Observable boundary:**
- The Handoff Artifact section names exactly three sections: Problem Statement, Consensus Evidence, Exit Criteria (case-insensitive match)
- The Consensus Evidence subsection identifies four evidence types: codebase / practitioner-friction (or equivalent) / philosophy (or Chester philosophy) / industry (or industry prior art)
- The Consensus Evidence subsection requires per-bullet attribution to a source pole (N/S/E/W)
- The Problem Statement subsection requires a single sentence
...
**Then:** all three sections are named, evidence types match, attribution rule is present, single-sentence rule for problem statement is present, and ratification follows
...
- Each pole agent body contains text inheriting or referencing the Software Architect persona from the Socratic Interview Improvement work (e.g., "reads code as design history", "thinks in trade-offs", "operates across abstraction levels", or equivalent persona traits)
...
**Observable boundary:**
- The swap-line block (currently `ACTIVE_UNDERSTANDING_MCP` or `ACTIVE_UNDERSTANDING_FLOW` — verify ground-truth) lists `team-interview` as one of the available options
- Whatever the swap-line variable is currently named, the new option appears in its valid-values comment block

**Given:** SKILL.md is modified
**When:** the test inspects the swap-line block
**Then:** `team-interview` is listed as a valid option
...
### AC-3.2 — `SKILL.md` Phase 3 / Phase 4 references the new flow file when active

**Observable boundary:**
- SKILL.md Phase 3 (Round One) and/or Phase 4 (Understand Stage) text references `references/team-interview-flow.md` (either by exact filename or via the existing path-templating pattern)
- The MCP-init step in Phase 3 is conditional: when `team-interview` is the active flow, no Understanding MCP initialization runs

**Given:** SKILL.md is modified
**When:** the test inspects Phase 3 and Phase 4 sections
**Then:** the flow file is referenced and MCP init is conditional
...
- Each row marks the policy as "named subagent, never fork" (or equivalent)
- Each row's rationale references framing-side (not review-side) dispatch and the need to isolate poles from each other and from lead framing
...
### AC-6.1 — Classic flow file is unmodified

**Observable boundary:**
- `skills/design-large-task/references/classic-mcp-flow.md` content is byte-identical to the version on `main` at the time the sprint branched (i.e., not modified during this sprint)
- The architectural flow file (`architectural-mcp-flow.md`) is also unmodified

**Given:** the sprint branch is compared to `main`
**When:** the test diffs the two existing flow files
**Then:** no changes appear in either

**Test skeleton ID:** `ac-6-1-classic-unchanged`

**Implementing tasks:**
**Decisions:**

### AC-6.2 — `design-large-task/SKILL.md` body sections for Phases 1, 2, and 5 are unchanged

**Observable boundary:**
- The Phase 1 (Bootstrap), Phase 2 (Parallel Context Exploration), and Phase 5 (Closure / Solve Stage section if applicable) bodies in SKILL.md show no diff against `main` other than possible swap-line adjustments
- Specifically: no semantic changes to the Bootstrap workflow, no semantic changes to the parallel exploration mechanics, no semantic changes to the Solve Stage handoff or proof-mcp-flow integration
