...
**Observable boundary:**
- The flow file's Per-Round Phases section (or a dedicated Dispatch Visibility subsection) names the orchestrator-side dispatch/completion printing convention from sprint 2026-03-31
- The instruction explicitly states that pole subagent dispatches must surface dispatch and completion lines on the main screen (not buried in subagent output panels)
- The convention is referenced for both per-phase pole dispatches in R1–R4 and the parallel synthesis attacks in R5

**Given:** the flow file exists
**When:** the test greps for the dispatch/completion printing convention reference
**Then:** the convention is documented and tied to the pole-dispatch instructions

**Test skeleton ID:** `ac-5-2-orchestrator-printing`
