...
**Revision:** 05 — addresses two plan-smell findings from plan-smell pass on plan-01:
1. **Integration test split legitimized.** Testing Strategy updated to direct integration tests to split into four focused bash scripts (capture-finalize, supersede, abandon, cross-sprint) plus a shared fixtures helper plus an AC-mapping verifier — instead of a single monolithic script. Addresses plan-smell responsibility-overload finding: a single script carries too many unrelated concerns, and a change in one concern fails to isolate from unrelated concerns in the same file. Splitting aligns each script with one NC/Rule family, keeping failure attribution clean.
2. **Section 2.1 insertion reaffirmed (plan-smell recommendation overruled).** Plan-smell recommended promoting the decision-record loop step to a new Section 2.2 in `execute-write/SKILL.md` (separate-section responsibility boundary). Reviewed and declined: spec-02 already settled this placement; Section 2.1 (Dispatch Pattern) is the ordered step sequence `execute-write` follows per task, and the decision-record trigger check belongs in-sequence with the other per-task steps (implementer dispatch, status handling, spec-compliance review). Promoting to a peer section would detach it from the task-step flow, adding navigation cost without clarifying responsibility. Placement stays at Section 2.1 step insertion between current steps 2 and 3 per spec-04.
3. **Test/Code composite-string format (plan-smell finding #1) — deferred, preserved as-is.** The `{test_name} @ {commit_sha}` and `{file:line} @ {commit_sha}` string formats introduced by `dr_finalize_refs` are a cosmetic smell (composite strings parsed via split), not a structural issue. Schema validates format; audit tooling parses correctly. Upgrade to structured sub-fields is tracked as future work; not in scope for this sprint.

Prior revisions:
- 04 addressed two CRITICAL + two HIGH findings from plan-attack: store path reinterpreted (directory + file per designer clarification), store path parameterization (`storePath` option + `CHESTER_DECISION_RECORD_PATH` env var), late-binding Test/Code SHA via new `dr_finalize_refs` tool, test framework aligned to `vitest` + `__tests__/`.
...
- **Skill integration tests:** bash-based per Chester convention, split into four focused scripts plus a shared fixtures helper plus an AC-mapping verifier. Each script isolates one concern so a failure attributes cleanly to one NC/Rule family (plan-smell responsibility-overload finding, rev 05):
  - `tests/test-decision-record-capture-finalize.sh` — capture path, schema-phase validation, `dr_finalize_refs` SHA-append idempotence, `dr_verify_tests` SHA-presence gate. Covers AC-1.1, AC-3.1, AC-4.1, AC-4.2.
  - `tests/test-decision-record-supersede.sh` — append-only supersede chain with bidirectional back-links. Covers AC-7.1.
  - `tests/test-decision-record-abandon.sh` — Active → Abandoned transition; Superseded preserved. Covers AC-11.1.
  - `tests/test-decision-record-cross-sprint.sh` — two-sprint simulation; `dr_query` returns prior records; must-remain-green constraints carry forward; `dr_audit` detects drift. Covers AC-5.1, AC-5.2, AC-8.1, AC-10.2.
  - `tests/test-decision-record-shared-fixtures.sh` — helper sourced by the four scripts above (mock sprint scaffolder, temp-store initializer, `CHESTER_DECISION_RECORD_PATH` setup). Not run as a standalone test.
  - `tests/test-decision-record-ac-mapping.sh` — verifies that every AC in the spec is exercised by at least one integration script (reads AC IDs from spec-05 and greps the four scripts for coverage). Guards against silent AC orphans.
  All integration scripts use `CHESTER_DECISION_RECORD_PATH` pointing at a per-test temp file; none write to the production store.
