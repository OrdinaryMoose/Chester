...
**Revision:** 01 — addresses three MEDIUM findings from adversarial review (path ambiguity, verify-tests ordering, language-awareness) plus three advisory items from fidelity review (path wording, cross-reference table, test-generator template file).
...
**Location (literal per RULE-1):** `/docs/chester/decision-record.md` — a single flat markdown file, sibling to `docs/chester/plans/` and `docs/chester/working/`. Memory.md-style single-file layout.
...
implementer subagent returns DONE/DONE_WITH_CONCERNS + observable-behaviors.md
   ↓
execute-write Section 2.1.5 reads artifact + spec skeleton manifest
   ↓
...
**Then:** `dr_capture` is invoked within the same task, before spec-reviewer dispatch; record is present in `/docs/chester/decision-record.md` with Status=Active
...
**Then:** writes land at `/docs/chester/decision-record.md`; file is created if absent on first call; no directory is used
...
|---|---|---|
| NC-1 (prospective capture) | execute-write Section 2.1.5 + `dr_capture` MCP tool | AC-1.1 |
...
| Rule: agent-autonomous capture | `execute-write` Section 2.1.5 (no human gate) | AC-1.1 |
