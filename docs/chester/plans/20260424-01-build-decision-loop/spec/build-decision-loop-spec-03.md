...
**Revision:** 03 — restructured to conform to the loop-optimized spec format this spec itself defines: every acceptance criterion now carries `Implementing tasks:` and `Decisions:` placeholder fields (populated downstream by plan-build and execute-write respectively). No architectural or content changes; structural reformatting only.

Prior revisions:
- 02 addressed 2 HIGH + 3 MEDIUM findings from ground-truth review (directory collision, hook-surface claim, skill-index path, execute-write section numbering, memory.md analogy).
