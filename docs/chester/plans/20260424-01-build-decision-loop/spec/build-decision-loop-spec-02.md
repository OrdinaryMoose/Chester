...
**Revision:** 02 — addresses 2 HIGH + 3 MEDIUM findings from ground-truth review: (1) pre-existing empty directory at `/docs/chester/decision-record/` must be removed before flat-file creation; (2) brief's "SessionStart only" hook-surface claim reflects refactor-chester-skills branch state, not main — decision-record design does not depend on hook surface regardless; (3) setup-start registration path corrected to `references/skill-index.md`; (4) execute-write insertion clarified as new numbered step in Section 2.1's Dispatch Pattern list; (5) memory.md analogy removed (not present in Chester repo).

Prior revisions: 01 addressed three MEDIUM findings from adversarial review (path ambiguity, verify-tests ordering, language-awareness) plus three advisory items from fidelity review (path wording, cross-reference table, test-generator template file).
...
- `dr_verify_tests(sprint)` — queries all records with `Sprint = {sprint}`; for each, verifies the test named in `Test` field exists in the test corpus and currently passes; returns per-record verdicts and aggregate pass/fail.
...
- `dr_verify_tests(sprint)` returns `{ sprint: string, per_record: [{ id, test, exists: bool, passes: bool }], aggregate: "pass" | "fail" }`
- `dr_audit(filter?)` returns `{ audited: int, drifted: int, findings: [{ id, kind: "test-missing" | "test-failing" | "code-moved", detail: string }] }`
...
**Location (literal per RULE-1):** `/docs/chester/decision-record.md` — a single flat markdown file, sibling to `docs/chester/plans/` and `docs/chester/working/`. Single-file layout: all records append into the one file as H2 sections.

**Pre-existing empty directory collision.** A directory at `/docs/chester/decision-record/` (no extension) already exists in the repository from earlier exploratory work. The spec requires the flat file at `/docs/chester/decision-record.md` (with extension). The first implementation task of this sprint **must remove the empty directory** (`rmdir docs/chester/decision-record`) before any MCP tool call attempts to create the flat file. If the directory is non-empty at implementation time, escalate to the designer — it was intended to be empty. Verified at AC-6.1.

**No supporting state files.** ID counter is stateless: at `dr_capture` time, `store.js` scans the existing file for the latest `YYYYMMDD-XXXXX` entry matching today's date and increments XXXXX. First record of the day starts at `00001`. Under concurrent-session access, `store.js` uses a file-lock on the store file itself during the read-increment-append sequence.

Created on first `dr_capture` call if absent. Tracked in git.
...
- Section 2.1 (Dispatch Pattern) currently has an ordered list of steps 1–6. Insert a new numbered step between current step 2 (Handle implementer status codes) and current step 3 (Dispatch spec compliance reviewer), renumbering subsequent steps accordingly. The new step is titled "Decision-Record Trigger Check and Propagation" and contains:
  ...
  - On FIRE: call `dr_capture` with full field set; run propagation procedure per `propagation-procedure.md` (spec-clause update, spec-driven test generation via `test-generator.md` subagent, full suite run via `execute-prove`); task is BLOCKED until suite passes including new test(s).
...
- **MCP tool tests:** unit tests for each tool (`dr_capture` field validation, `dr_supersede` append-only semantics, `dr_query` filter logic, `dr_abandon` status transitions, `dr_verify_tests` integration, `dr_audit` drift detection). Tests in `tests/mcp/chester-decision-record/`.
...
**Given:** execute-write runs a task whose implementer surfaces a decision ambiguity; pre-existing empty directory `/docs/chester/decision-record/` has already been removed by the sprint's setup step
**When:** implementer returns DONE/DONE_WITH_CONCERNS with observable-behaviors.md reporting uncovered behavior
**Then:** `dr_capture` is invoked within the same task, before spec-reviewer dispatch; record is present in `/docs/chester/decision-record.md` (single flat file) with Status=Active
...
### AC-4.2 — Sprint-end record-test linkage verified after suite pass

**Observable boundary:**
- every record created this sprint has a Test field pointing to a passing test in the corpus AND `execute-prove` suite passes → `execute-verify-complete` proceeds to clean-tree check
- any record's Test field is broken → checkpoint commit blocked with record-ID-specific error
...
**Then:** if any record has a missing or failing test, verify-complete BLOCKS with structured error listing record IDs; no clean-tree check or checkpoint commit until resolved
...
### AC-6.1 — Persistent store at /docs/chester/decision-record.md (literal flat file)

**Observable boundary:**
- pre-existing empty directory at `/docs/chester/decision-record/` has been removed before any MCP write occurs
- `dr_capture` writes to `/docs/chester/decision-record.md` only (single file at literal RULE-1 path)
- no other write path accepts records
- no supporting sidecar files are created

**Given:** MCP `store.js` hardcodes write target; sprint's setup task has removed any pre-existing directory at the bare path
**When:** any MCP tool is invoked with record-writing intent
**Then:** writes land at `/docs/chester/decision-record.md`; file is created if absent on first call; no directory exists at `/docs/chester/decision-record/` (bare, no extension) at implementation completion
...
**Then:** record is visible in `/docs/chester/decision-record.md` immediately; no intermediate location; no sprint-close promotion step required for queryability
...
- decision-record MCP's state file at `/docs/chester/decision-record.md` is disjoint from proof/understanding MCP state paths
...
| NC-6 (store location) | `store.js` hardcoded target `/docs/chester/decision-record.md` | AC-6.1 |
