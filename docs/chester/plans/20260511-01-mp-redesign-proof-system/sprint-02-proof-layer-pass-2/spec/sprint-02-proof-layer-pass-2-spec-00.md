...
### AC-2.3 — Status transition rule fires on ratify
...
**Then:** the query succeeds. The `'draft'` row may also still be present (rule-derivation, not retraction) — the AC requires only that the `'ratified'` row is derivable.
...
<!-- created-at: 2026-05-15T01:25:09Z -->
