...
- `-02` (this revision): ground-truth review surfaced HIGH-1 (AC-1.4's idShape→short-prefix mapping premise is false — existing allocator emits long-form `${idShape}_${n}` not the cascade-shorthand `cern_N`) and MEDIUM-2 (AC-5.1's universal grep boundary conflicts with AC-5.3's four-file enumeration — `state.concerns` is also read in `server.js`, `metrics.js`, `friction-detection.js`). AC-1.4 regex relaxed to `^concern_\d+$`; AC-5.3 expanded to enumerate all consumer sites; AC-1.4 citation tightened to `state.js:253`. No architectural change.
...
### AC-5.3 — All `state.concerns` consumers rewired

**Observable boundary:**
- The four downstream readers `closing-argument.js`, `body-advancement.js`, `first-yes-gate.js`, and `state-render.js` no longer read from `state.concerns`. They obtain Concern data via the bridge (e.g., `domain-bridge.queryProof({ pattern: ['concern_status', ...] })` or whatever query primitive the plan settles on).
- **Additionally** (surfaced by ground-truth review): the following consumers also read `state.concerns` and must be rewired the same way:
  - `server.js` at the four sites that read concerns for completeness counts, ratification counts, proof-state assembly, and closing-argument readiness (ground-truth review cited `server.js:427,452,463,489` — verify exact lines at plan time).
  - `metrics.js` at the six sites that read concerns for coverage math and two-yes preconditions (ground-truth review cited `metrics.js:160,163,271,288,320,371`).
  - `friction-detection.js` — currently called with `state.concerns` as a parameter; its caller must be updated to pass the bridge-derived concern list (or to query the bridge from inside `friction-detection.js`, plan's choice).
- For any proof state previously expressible via `state.concerns`, every rewired consumer's outputs are byte-identical to their pre-spec outputs.

**Given:** the post-spec proof-mcp consumers and a proof state with Concerns added through the bridge.
**When:** each consumer runs against that state.
...
<!-- created-at: 2026-05-15T01:38:55Z -->
