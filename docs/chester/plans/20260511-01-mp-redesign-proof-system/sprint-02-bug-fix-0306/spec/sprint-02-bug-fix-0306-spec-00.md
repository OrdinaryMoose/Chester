...
**Given:** the EVIDENCE descriptor in `schema.js`
**When:** descriptor is inspected by a test
**Then:** `closedEnumFields.source` resolves to the four-value spec-allowed set; `'designer'` is excluded
...
- `closedEnumFields` includes `friction_shape` and `disposition`
...
### AC-9.1 — renderStructuredProof PROPOSITION block surfaces inference pattern
...
**Given:** a bridge state with at least one PROPOSITION element
**When:** `renderStructuredProof` is called
**Then:** the PROPOSITION's primary line is followed by an `Inference pattern:` sub-line carrying the underscore-form value
...
### AC-9.3 — renderStructuredProof FRICTION block surfaces friction_shape + anchors

**Observable boundary:**
- For any FRICTION element, the rendered output contains sub-lines: "Friction shape: <friction_shape>", "Anchor A: <anchor_a-id>", "Anchor B: <anchor_b-id>", "Disposition: <disposition>"

**Given:** a bridge state with at least one FRICTION element with valid fields
**When:** `renderStructuredProof` is called
**Then:** the FRICTION's primary line is followed by the four sub-lines
