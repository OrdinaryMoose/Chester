...
Close the residual cascade-spec drift between `docs/chester/working/20260511-01-mp-redesign-proof-system/design-documents/05-domain-spec.md` and the proof-system implementation at `skills/design-proof-system/references/domain/`. The cascade-spec probe at `docs/chester/working/stress-tests/20260517-01/cascade-spec-probe-simulation.mjs` reports 16 failures against main after sprint-02-bug-fix-02 merge; this sprint drives that count to 0 across five category territories (EVIDENCE, PROPOSITION, RESOLUTION, FRICTION, DEFINITION) plus two cascade-document edits (DEF-8 §3.5 risk arity, D5-driven §3.4 inference_pattern propagation). Inline declarative-directive machinery established by bug-fix-01 (`closedEnumFields`, `nonEmptyStringFields`) and bug-fix-02 (`nonEmptyArrayFields`, `referenceFields`, `INVALID_REFERENCE`, `_CATEGORY_PROBES_SCHEMA`) is the load-bearing infrastructure — this sprint extends it to five more category descriptors and adds the parallel test files, without restructuring it.
...
- `docs/chester/working/20260511-01-mp-redesign-proof-system/design-documents/05-domain-spec.md` §3.4.1 — five inference_pattern values updated to underscore form, `proposition_composition` added.
- `docs/chester/working/20260511-01-mp-redesign-proof-system/design-documents/05-domain-spec.md` §3.4 main text — `inference_pattern` status flipped from "Optional but encouraged" to "Required" to match impl.
- `docs/chester/working/20260511-01-mp-redesign-proof-system/design-documents/05-domain-spec.md` §3.5 — `risk(RiskId, Statement)` updated to `risk(RiskId, Statement, Severity)` (DEF-8 closure).
...
- `skills/design-proof-system/references/domain/__tests__/evidence-schema.test.js` — real-import test file following permission-schema.test.js / risk-schema.test.js precedent.
...
- `skills/design-proof-system/references/domain/__tests__/proposition-schema.test.js` — extended (existing file) with D2 grounding spread and D5 enum coverage.
...
- **Render layer tests** — extended `skills/design-proof-system/references/domain/__tests__/render.test.js` (existing file) for the new sub-lines if not already covered by per-category bridge round-trips.
...
- **Scope discipline** — changes confined to: `skills/design-proof-system/references/domain/{tags,schema,mutations,translation,render}.js`, `skills/design-proof-system/references/domain/__tests__/`, `docs/chester/working/stress-tests/20260517-01/`, and `docs/chester/working/20260511-01-mp-redesign-proof-system/design-documents/05-domain-spec.md` §3.4 and §3.5 only. No other cascade sections; no files outside these directories.
...
**Observable boundary:**
- `docs/chester/working/20260511-01-mp-redesign-proof-system/design-documents/05-domain-spec.md` §3.5 text contains the fact form `risk(RiskId, Statement, Severity)` (arity 3)
...
**Observable boundary:**
- `docs/chester/working/20260511-01-mp-redesign-proof-system/design-documents/05-domain-spec.md` §3.4.1 lists exactly five inference_pattern values, all in underscore form: `grounds_imply_conclusion`, `rule_applies_to_case`, `permission_licenses_relaxation`, `definition_substitution`, `proposition_composition`
...
**Observable boundary:**
- `docs/chester/working/20260511-01-mp-redesign-proof-system/design-documents/05-domain-spec.md` §3.4 main text describes `inference_pattern` as required (not "Optional but encouraged" or equivalent)
...
- These test files exist and follow the bug-fix-02 pattern (real-import, `createDomainBridge`, `makeRealBridge` factory):
  - `skills/design-proof-system/references/domain/__tests__/evidence-schema.test.js`
  - `skills/design-proof-system/references/domain/__tests__/resolution-schema.test.js`
  - `skills/design-proof-system/references/domain/__tests__/friction-schema.test.js`
  - `skills/design-proof-system/references/domain/__tests__/definition-schema.test.js`
- The fifth category test file (`skills/design-proof-system/references/domain/__tests__/proposition-schema.test.js`) already exists and is *extended* rather than created — see AC-11.2.

**Given:** the post-sprint codebase
**When:** the `skills/design-proof-system/references/domain/__tests__/` directory is scanned
...
**Observable boundary:**
- `skills/design-proof-system/references/domain/__tests__/proposition-schema.test.js` contains tests asserting:
...
- Files changed by this sprint are confined to:
  ...
  - `skills/design-proof-system/references/domain/__tests__/*.test.js` (new and modified per-category files; render tests if needed)
  - `docs/chester/working/stress-tests/20260517-01/cascade-spec-probe-simulation.mjs`
  - `docs/chester/working/20260511-01-mp-redesign-proof-system/design-documents/05-domain-spec.md` §3.4 and §3.5 only
