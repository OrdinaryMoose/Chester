...
**Revision note (spec-02):** aligned to the merged Engine public API from sprint-01 pass 4 (commit 146bc68). Three corrections vs. spec-01:
1. Counterfactual data flow §11.1 — replaced the unimplementable wildcard `retractFact("approved", [propId, "_", "_"])` with the query-then-retract pattern, since Engine `retractFact` requires exact constant args.
2. Added the "Engine public-surface signatures" component section pinning `defineRule(ruleId, headAtom, bodyAtoms, metadata)` (4-arg tuple form, Engine Spec §4.2), `explain(fact)` (1-arg, Engine Spec §4.5), and the rest of the §4 surface — so the Domain implementer does not re-discover the API sprint-01 spent four passes converging on.
3. Fixed Engine Spec citation `§4.5 line 305` → `§4.6 line 305` for the snapshot/restore bit-equal property.
...
### Engine public-surface signatures (consumed by port bundles)

The port bundles wrap the Engine public API per `04-engine-spec.md` §4 as realized in the merged Engine (commit 146bc68 — sprint-01-proof-backend pass 4). The signatures below are **normative for sprint-02**; any port-bundle adapter and any `translate()` rule shape must produce calls in exactly this form. (Sprint-01 spent four passes converging on this surface; the spec pins it here so the Domain layer does not re-discover it.)

- `WritePorts.rules.defineRule(ruleId, headAtom, bodyAtoms, metadata)` — **4-arg tuple-form** per Engine Spec §4.2. `headAtom` is `[predicate, [...args]]`; `bodyAtoms` is an array of `[predicate, [...args]]` (or negated form per the engine's body-atom grammar); `metadata` is the rule's metadata object. Domain `translate()` outputs each rule in `RuleSpec` shape `{ruleId, headAtom, bodyAtoms, metadata}` and `mutations.js` / `translation.instantiateTemplate` destructures into the 4-arg call.
- `WritePorts.rules.undefineRule(ruleId)` — 1-arg per Engine Spec §4.2.
- `WritePorts.facts.assertFact(predicate, args)` / `WritePorts.facts.retractFact(predicate, args)` — 2-arg per Engine Spec §4.1. `args` must be a tuple of exact constants (string, finite number, boolean, or null). Wildcards are **not** accepted by `assertFact` or `retractFact`; pattern-driven retract uses the query-then-retract form in the Counterfactual execution data flow.
- `ProbePorts.snapshot.snapshot() → token` / `ProbePorts.snapshot.restore(token)` per Engine Spec §4.6. `token` is opaque; the Engine guarantees bit-equal restore.
- `WritePorts.query.query(pattern)` / `count(pattern)` / `exists(pattern)` / `derive()` per Engine Spec §4.4 + §4.3. `pattern` is `[predicate, [...args]]` where each arg is a constant or the wildcard string `"_"`; wildcards bind nothing (Engine Spec §9.3). `derive()` is normally implicit — every `query` calls it lazily under read-own-writes (Engine Spec §4.8, ADR-0013 Part 2) — and is also exposed for explicit triggers in `runOperation` step 6.
- `ReadPorts.explain(fact)` — **1-arg** per Engine Spec §4.5; `fact` is a `[predicate, args]` tuple. Returns the derivation tree or the documented "not derived" sentinel.
- `WritePorts.tx.begin()` returns a `Symbol` handle; `WritePorts.tx.commit(handle)` / `WritePorts.tx.rollback(handle)` per Engine Spec §4.8. The handle is opaque to the Domain and must be passed back unmodified.

The Engine throws structured `{code, ...}` errors (e.g., `STALE_HANDLE`, `NESTED_TRANSACTION`, `CYCLIC_NEGATION`, `DUPLICATE_RULE_ID`, `TYPE_ERROR`) at violations of these contracts; the bridge surfaces them as `DomainBootError` (boot-time `defineRule` failures) or lets them propagate inside `runOperation`'s try-block to be caught alongside transaction-scoped errors per the rollback discipline.
...
2. Try, in a try/finally where the `finally` always runs `probePorts.snapshot.restore(snap)`:
   - The §11.1 pattern is **query-then-retract**: `const matches = probePorts.query.query(["approved", [propId, "_", "_"]])` returns binding sets for every concrete `approved(propId, X, Y)` fact (wildcards `"_"` are supported in query patterns per Engine Spec §9.3); iterate the matches and call `probePorts.facts.retractFact("approved", concreteArgs)` for each. Engine `retractFact(predicate, args)` requires **exact constant args** (Engine Spec §4.1; matched by `JSON.stringify(args)` lookup, not by pattern). The cascade's `05-domain-spec.md:723` pseudocode `engine.retractFact("approved", [PropId, _, _])` is descriptive of intent; the implementable form is query-then-retract. Other counterfactual flavors per §11.2 call `assertFact` for hypothetical-fact-addition queries.
...
3. The `finally` runs regardless of throw or return: `probePorts.snapshot.restore(snap)` reverts engine state to pre-call (bit-equal per Engine Spec §4.6 line 305).
...
- Architecture §10 payoffs → split:
  - "Engine reusable", "Domain testable without Interface", "Interface replaceable" — verified at sprint/layer-boundary scope; not test-targeted in sprint-02.
  - "Counterfactuals come for free" — verified by AC-7.1 (snapshot/restore-on-throw).
  - "Independent verification path" (Datalog projection consumable by a second engine) — verified by AC-11.1's Datalog-projection sub-clause.
  - "Adversary integrates cleanly" — verified by AC-11.1's read-only audit adapter sub-clause (a representative Adversary-shape client using only `ReadPorts`).
  - The brief's framing "narrow-port audit adapter clean" paraphrases §10's "Independent verification path" + "Adversary integrates cleanly" through the lens of ISP discipline. The cascade's literal payoff names (`03-architecture.md:459-464`) are restored here for accuracy.
...
### AC-11.1 — Architecture §10 payoffs mechanically realized

**Observable boundary (audit-adapter sub-clause — "Adversary integrates cleanly"):**
...
**Observable boundary (Datalog-projection sub-clause — "Independent verification path"):**
- `IRenderSurface.renderDatalogProjection(args, readPorts)` returns a serializable Datalog projection of current proof state — base facts + rule heads + derived facts — in a format that a second engine (the cascade names this as the "independent confirmation" surface in `03-architecture.md:463`) could load and re-derive against.
- The output is parseable (round-trip through `JSON.stringify`/`JSON.parse` preserves it; alternative Datalog text serialization is also acceptable per the cascade's text/JSON dual mode).

**Given:** the bridge module exports `createReadOnlyAudit`; an in-memory substrate is populated via a normal bridge with sample mutations.
**When:** `bridge-integration.test.js` (`audit-adapter` case) iterates every render method AND (`datalog-projection` case) calls `renderDatalogProjection` and inspects the serialized output.
**Then:** every render method returns a payload; no mutation method is callable on the audit adapter; the Datalog projection output is parseable and structurally complete.
...
<!-- revised-at: 2026-05-13T22:10:00Z -->
<!-- revision: spec-02 aligns Engine references to merged sprint-01 pass-4 API (commit 146bc68) -->
