...
**Spec:** `docs/chester/working/20260511-01-mp-redesign-proof-system/sprint-02-proof-layer/spec/sprint-02-proof-layer-spec-01.md`
...
export function createInMemorySubstrate() {
  ...
  const explainPort = {
    explain(fact) { return { fact, derivation: [], provenance: 'in-memory-fake' }; },
  };
  ...
  function _unify(pat, fact) {
    ...
    for (let i = 0; i < pat.length; i++) {
      if (pat[i] === '_') continue;
      if (typeof pat[i] === 'string' && /^[A-Z]/.test(pat[i])) { b[pat[i]] = fact[i]; }
      else if (pat[i] !== fact[i]) return null;
    }
    return b;
  }
  ...
  return { facts, rules: rulesPort, query: queryPort, snapshot: snapshotPort, explain: explainPort, tx };
}
```
...
// rules:     Array<{ruleId, head, body, metadata}> — defineRule inputs (parameterized).
...
export function registerRuleTemplates(rulePorts) {
  ...
  for (const [cat, template] of Object.entries(RULE_TEMPLATES)) {
    ...
    rulePorts.defineRule(r.ruleId, r.head, r.body, { ...r.metadata, isTemplateAnchor: true });
  }
}
...
export function instantiateTemplate(idShape, newId, rulePorts) {
  ...
  rulePorts.defineRule(r.ruleId, r.head, r.body, r.metadata);
}
...
export function getRound(ports) {
  const rows = ports.query.query(['round', ['N']]);
...
export function getPhase(ports) {
  const rows = ports.query.query(['phase', ['P']]);
...
export function triggerGate(args, readPorts) {
  if (!readPorts.query.exists(['closure_permitted', []])) {
    const reasons = readPorts.query.query(['closure_failure_reason', ['R']]).map(b => b.R);
...
export function detectFrictions(readPorts) {
  const out = [];
  for (const row of readPorts.query.query(['ungrounded_proposition', ['P']])) out.push({ shape: FRICTION_SHAPES.UNGROUNDED, args: [row.P] });
  for (const row of readPorts.query.query(['coverage_gap_detected', ['C']])) out.push({ shape: FRICTION_SHAPES.COVERAGE_GAP, args: [row.C] });
  for (const row of readPorts.query.query(['overlap_detected', ['T1', 'T2']])) out.push({ shape: FRICTION_SHAPES.OVERLAP, args: [row.T1, row.T2] });
  for (const row of readPorts.query.query(['conflict_detected', ['R1', 'R2']])) out.push({ shape: FRICTION_SHAPES.CONFLICT, args: [row.R1, row.R2] });
  return out;
}
```
...
export function renderElementDeep(args, readPorts) {
  const { id } = args;
  if (!id) return null;
  // Query all predicates whose first arg is id. For brevity, check known predicates.
  for (const pred of ['evidence', 'rule_decl', 'permission_decl', 'proposition_decl', 'risk', 'resolution_decl', 'friction', 'definition_decl']) {
    const rows = readPorts.query.query([pred, [id, ...Array(5).fill('_')]]);
...
export function renderDatalogProjection(args, readPorts) {
  ...
  const facts = [];
  for (const pred of ['evidence', 'rule_decl', 'permission_decl', 'proposition_decl', 'resolution_decl', 'definition_decl', 'risk', 'friction', 'approved']) {
    for (const row of readPorts.query.query([pred, Array(8).fill('_')])) {
      facts.push([pred, Object.values(row)]);
    }
  }
...
export function getProofState(args, readPorts) {
  return {
    evidence: readPorts.query.query(['evidence', ['I', 'S', 'C']]),
    propositions: readPorts.query.query(['proposition', ['I', 'S']]),
    resolutions: readPorts.query.query(['resolution', ['I', 'S']]),
...
**Background:**
`collapseTest(args, probePorts)` implements Domain Spec §11.1: `snapshot → retractFact("approved",...) → derive → query closure_permitted → restore`. The `restore(snap)` MUST run in a `finally` so engine state reverts on any throw (AC-7.1). `queryWith(args, probePorts)` and `queryWithout(args, probePorts)` per §11.2. ProbePorts includes `facts` (per spec amendment).
...
describe('counterfactual', () => {
  ...
  it('queryWithout / queryWith run with snapshot bracketing', () => {
    const s = createInMemorySubstrate();
    expect(() => queryWithout({ retract: [['approved', ['prop_1', '_', '_']]], pattern: ['closure_permitted', []] }, { query: s.query, explain: s.explain, snapshot: s.snapshot, facts: s.facts })).not.toThrow();
    expect(() => queryWith({ assert: [['evidence', ['e1', 'src', 'c']]], pattern: ['evidence', ['I', 'S', 'C']] }, { query: s.query, explain: s.explain, snapshot: s.snapshot, facts: s.facts })).not.toThrow();
  });
});
```
...
export function collapseTest(args, probePorts) {
  ...
  try {
    probePorts.facts.retractFact('approved', [args.propId, '_', '_']);
    const stillCloses = probePorts.query.exists(['closure_permitted', []]);
    const reasons = stillCloses ? [] : probePorts.query.query(['closure_failure_reason', ['R']]).map(b => b.R);
...
export function queryWithout(args, probePorts) {
  const snap = probePorts.snapshot.snapshot();
  try {
    for (const [pred, a] of args.retract) probePorts.facts.retractFact(pred, a);
...
describe('boot-validators', () => {
  ...
  it('validateRuleTemplates throws on unresolved elementCategory', () => {
    const bad = { ...RULE_TEMPLATES, junk: { elementCategory: 'not_a_category', build: () => ({ ruleId: 'x', head: [], body: [] }) } };
...
export function runOperation(verbName, args, consent, ports) {
  ...
  try {
    ...
    for (const r of rules) ports.rules.defineRule(r.ruleId, r.head, r.body, r.metadata);
...
export function createDomainBridge({ engine, clock, idAllocator, consentVerification, persistenceRepo }) {
  ...
  try { registerRuleTemplates(writePorts.rules); }
    ...
    throw new DomainBootError({ validator: 'Phase B registerRuleTemplates', recordId: 'RULE_TEMPLATES', field: '*', violation: e.message, cause: e });
  }
...
export function createDomainBridgeWith(deps, overrides = {}) {
  ...
  try {
    for (const [, template] of Object.entries(templates)) {
      ...
      const r = template.build(placeholder);
      writePorts.rules.defineRule(r.ruleId, r.head, r.body, { ...r.metadata, isTemplateAnchor: true });
    }
  } catch (e) {
    throw new DomainBootError({ validator: 'Phase B registerRuleTemplates', recordId: 'RULE_TEMPLATES', field: '*', violation: e.message, cause: e });
  }

  // Returned facade omitted here for brevity — implementer copies the facade block from createDomainBridge.
  // For the AC-4.x tests, reaching this point means construction succeeded; the tests only assert the throw paths.
  return Object.freeze({ /* facade methods — copy from createDomainBridge */ });
}

/**
 * Read-only audit adapter. Exposes only render and query methods over readPorts.
...
describe('module-shape', () => {
  ...
  it('all 13 module test files exist', () => {
    const tests = readdirSync(resolve(DOMAIN, '__tests__')).filter(f => f.endsWith('.test.js'));
    for (const stem of NAMED) expect(tests).toContain(stem.replace('.js', '.test.js'));
  });
...
function makeAdapters({ failSaveOnce = false } = {}) {
  ...
  return {
    clock: { now: () => 1700000000 },
    idAllocator: { next: (p) => `${p}_${Math.random().toString(36).slice(2)}` },
    consentVerification: { verify: () => true },
...
describe('bridge-integration', () => {
  it('AC-3.4 — runOperation port-call ordering for addElement matches §6.1', () => {
    ...
    expect(order.indexOf('tx.begin')).toBeLessThan(order.indexOf('facts.assertFact'));
    expect(order.indexOf('facts.assertFact')).toBeLessThan(order.indexOf('tx.commit'));
  });
  ...
  it('AC-4.3 — DomainBootError thrown when bridge is constructed against RULE_TEMPLATES with unresolved elementCategory', async () => {
    ...
    const substrate = createInMemorySubstrate();
    const badTemplates = { ...RULE_TEMPLATES, junk: { elementCategory: 'not_a_category', build: (id) => ({ ruleId: `${id}_x`, head: ['x', [id]], body: [], metadata: {} }) } };
    expect(() => createDomainBridgeWith({ engine: substrate, ...makeAdapters() }, { ruleTemplates: badTemplates }))
      .toThrow(DomainBootError);
  });
  ...
  it('AC-4.3 / AC-5.1 (Phase B stratification sub-case) — cyclic-negation template body throws as DomainBootError at Phase B defineRule', async () => {
    ...
    const cyclicTemplates = {
      ...
      cyclic_test: {
        ...
        build: (id) => ({
          ruleId: `${id}_cyclic`,
          head: ['p', [id]],
          body: [['not', ['q', [id]]], ['p', [id]]], // p depends on not q AND on p — cyclic through negation
          metadata: { element: id },
        }),
      },
    };
    ...
    expect(() => createDomainBridgeWith({ engine: substrate, ...makeAdapters() }, { ruleTemplates: cyclicTemplates }))
      .toThrow(DomainBootError);
  });
  ...
  it('AC-11.1 — createReadOnlyAudit runs renders end-to-end and excludes mutations', () => {
    const substrate = createInMemorySubstrate();
    const bridge = createDomainBridge({ engine: substrate, ...makeAdapters() });
    bridge.addElement({ idShape: ELEMENT_CATEGORIES.EVIDENCE, source: 'codebase', claim: 'x' }, { source: CONSENT_SOURCES.DESIGNER });
    const audit = createReadOnlyAudit(substrate);
    expect(typeof audit.renderStructuredProof({})).toBe('string');
