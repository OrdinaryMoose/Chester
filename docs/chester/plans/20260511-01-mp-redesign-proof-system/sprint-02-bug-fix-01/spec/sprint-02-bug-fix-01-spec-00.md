...
- `skills/design-proof-system/references/domain/__tests__/bridge-integration.test.js` (and any other test files containing four-field PROPOSITION fixtures)
  - Fixture repair as above; the Explorer named `bridge-integration.test.js:103` as carrying the four-field PROPOSITION required-field comment.
...
**Ratify-path required-field exposure.** Per the Explorer report, the prior CONCERN sub-sprint identified that `verifyArgsShape` is called from the ratify path with `{elementId, idShape}` args — which lack the category's `requiredFields`. Growing PROPOSITION's required-field count from four to five compounds the exposure. AC-6.1 below verifies that `ratifyElement({elementId, idShape: 'proposition'}, consent)` does not throw `SHAPE_INVALID`. If it does, the fix is to short-circuit `verifyArgsShape`'s `requiredFields` loop when the call is a ratify-shape call rather than an add/revise-shape call. The mechanism (skip `requiredFields` for ratify-shape args, or split into separate descriptor shapes) is left to the implementer to choose during plan-build; this spec asserts only the observable behavior (no throw).
...
### AC-3.4 — Boot validation accepts the new emissions without warning

**Observable boundary:**
- Constructing a bridge via `createDomainBridge(...)` and asserting a PROPOSITION with `reasoning_chain` and a non-empty `rejected_alternatives` array completes without any boot-validator warning or error related to unknown EDB predicates.

**Given:** a freshly constructed bridge.
**When:** the test calls `addElement` for a full-shape PROPOSITION.
**Then:** the call returns successfully; no warning is emitted to whatever capture mechanism boot-validators use.
