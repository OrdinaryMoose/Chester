...
### Task 2: Update ~/.claude/settings.json hook path

**Files:**
- Modify: `~/.claude/settings.json` (user config, outside repo)

**Why:** The SessionStart hook points to `chester-hooks/session-start`. After Task 1 renamed that directory, any new Claude Code session would fail to load Chester. This must be done immediately after the directory rename.
...
### Task 3: Update `name:` frontmatter in each SKILL.md
...
### Task 4: Update cross-references inside skill content files

**Files:**
- Modify: all `.md`, `.sh`, and extensionless script files under `chester-*/` (post-rename directories)
...
```bash
FILES=$(find chester-*/ -type f \( -name "*.md" -o -name "*.sh" -o -name "session-start" \))
```

Note: `session-start` has no file extension — it must be explicitly included in the find pattern.
...
| `chester-start-debug` | `chester-setup-start-debug` |
...
- [ ] **Step 2: Remove dead `chester-doc-sync` reference**

Line 65 references `chester-doc-sync` — a skill that does not exist. Remove this row from the Utilities table.

- [ ] **Step 3: Update installation hook path**
...
Expected: no matches (all names are new-style, no dead references).
...
Same longest-first ordering as Task 4, scoped to `tests/*.sh`:
...
grep -rn 'chester-start-debug\|chester-figure-out\|chester-build-spec\|chester-build-plan\|chester-attack-plan\|chester-smell-code\|chester-write-code\|chester-test-first\|chester-fix-bugs\|chester-prove-work\|chester-review-code\|chester-finish-plan\b\|chester-write-summary\|chester-trace-reasoning\|chester-make-worktree\|chester-dispatch-agents\|chester-hooks' \
  --include="*.md" --include="*.sh" --include="*.json" --include="session-start" \
  --exclude-dir=docs \
  .
```
...
```bash
grep -rn 'chester-start' --include="*.md" --include="*.sh" --include="*.json" --include="session-start" --exclude-dir=docs . | grep -v 'chester-setup-start'
```
