...
# execute-write is at v0009 (off-roster subagent dispatch — workers self-dispose, no team_name, no TeamDelete leak).
CUR_VER="$(awk '/^version:/ {print $2; exit}' "$SKILL")"
[ "$CUR_VER" = "v0009" ] || fail "version not at v0009 (got $CUR_VER)"
