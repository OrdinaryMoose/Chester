...
grep -qiE "all recommendations (=|are) opinions" "$PARTNER" || { echo "FAIL AC-1.2: hard rule on recommendations missing"; exit 1; }
...
INTRO_BLOCK=$(awk '/^## Interpreter Frame/,/^## Private Precision Slot/' "$PARTNER")
...
grep -qE '^## Core Stance' "$PARTNER" || { echo "FAIL AC-5.2: Core Stance section missing"; exit 1; }
grep -qE '^## Interpreter Frame' "$PARTNER" || { echo "FAIL AC-5.2: Interpreter Frame section missing"; exit 1; }
...
# AC-4.1: design-large-task per-turn flow — RETIRED. The design-large-task skill was
# removed from the repo; this cross-reference check no longer has a target. AC-4.2
# (design-small-task) still exercises the per-turn C1/C2 contract on a live skill.
