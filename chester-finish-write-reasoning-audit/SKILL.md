...
description: >
  ...
  implementation session alongside chester-finish-write-session-summary. Also trigger explicitly when the user says
  things like: "write the reasoning audit", "audit the decisions", "how did you decide that",
  "capture the decision reasoning", "reasoning audit", or "/chester-finish-write-reasoning-audit".
  Do NOT trigger for planning-only or analysis-only sessions with no code changes — use
  chester-finish-write-session-summary alone for those.
---
...
chester-finish-write-reasoning-audit uses — both skills draw from the primary source independently. Do not
derive content from chester-finish-write-session-summary output or chester-finish-write-reasoning-audit output.
...
Find the current session's JSONL file using the same approach as chester-finish-write-reasoning-audit:
